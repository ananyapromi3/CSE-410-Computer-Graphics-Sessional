...
GLfloat xx = 0.0f;               // X position for the clock
GLfloat yy = 0.0f;               // Y position for the clock
std::vector<float> Px;
std::vector<float> Py;
...
void display()
{
    ...
    glPushMatrix();
    {
        ...
        glColor3f(1.0f, 1.0f, 1.0f); // Set color to red for second hand
        // drawSecondHands(0.0, 0.0, 0.5f);              // Draw the second hand
        drawLine(0.0, 0.0, xx, yy);
    }
    glPopMatrix();
    glPushMatrix();
    {
        drawLine(xx, yy, 0.6, yy);
    }
    glPopMatrix();
    ...
    glPushMatrix();
    {
        ...
        {
            j++;
            // glTranslatef(0.6 + i * 0.01, yy, 0.0);
            // glVertex2d(0.6, yy);
            // printf("x: %f, y: %f\n", Px[i], Py[i]);
            // drawCircle2(0.6 + i * 0.01, Py[i], 0.001);
            glVertex2d(0.6 + j * 0.01, Py[i]);
        }
        glEnd();
    }
    glPopMatrix();
...
void timerFunction(int value)
{
    ...
    secHandRotation += 45.0f * deltaSec;
    if (secHandRotation < 0.0f)
        secHandRotation += 360.0f;
    xx = 0.5f * cosf(secHandRotation * M_PI / 180.0f);
    yy = 0.5f * sinf(secHandRotation * M_PI / 180.0f);
    Px.push_back(xx);
    Py.push_back(yy);
    if (Px.size() > 1000)
    {
